...
const RecipeForm = () => {
  ...
  const uploadImageToImgBB = (file) => {
    ...
    axios.post('https://api.imgbb.com/1/upload?key=YOUR_IMG_BB_API_KEY', uploadData)
      .then((response) => {
      ...
      <div className="max-w-[1200px] p-4">
        ...
        {currentStep === 4 && (
          <div className='card min-w-80 h-[360px] border-2 p-4 rounded-md'>
            ...
            {/* Image placeholder */}
            <div
              className='img-box w-56 h-48 bg-transparent flex items-center justify-center rounded-md cursor-pointer'
              onClick={() => fileInputRef.current.click()}
            >
              {uploadedImage ? (
                <img src={uploadedImage} alt="Uploaded Image" className="w-full h-full object-cover rounded-md" />
              ) : (
                <svg
...
//     const file = e.target.files[0];
//     if (file) {
//       setFiles([...files, file]); // Update files state with selected file
//     }
//   };
